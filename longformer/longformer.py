...
from longformer.diagonaled_mm_tvm import (
    diagonaled_mm as diagonaled_mm_tvm,
...
class LongformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights_local, attn_weights_global = None, None
        ...
        if attention_mask is not None:
            ...
            max_num_extra_indices_per_batch = (
                num_extra_indices_per_batch.max().item()
            )
        ...
        if extra_attention_mask is not None:
            ...
            selected_attn_weights = torch.einsum(
                "blhd,bshd->blhs", (q, selected_k)
            )
            selected_attn_weights[
                selection_padding_mask_zeros[0],
                :,
                :,
                selection_padding_mask_zeros[1],
            ] = -10000
            # concat to attn_weights
            # (bsz, seq_len, num_heads, extra attention count + 2*window+1)
            attn_weights = torch.cat(
                (selected_attn_weights, attn_weights), dim=-1
            )
        ...
        if output_attentions:
            attn_weights_local = attn_probs.permute(0, 2, 1, 3)
        ...
        if extra_attention_mask is not None:
            ...
            if output_attentions:
                attn_weights_global = attn_probs.view(
                    bsz,
                    self.num_heads,
                    max_num_extra_indices_per_batch,
                    seq_len,
            ...
            ] = nonzero_selected_attn.view(
                len(selection_padding_mask_nonzeros[0]), -1
            ).type_as(
                hidden_states
            )

        return (attn, attn_weights_local, attn_weights_global)
